...
public class RootResource {
    ...
    private static final Pattern YESNO_QUESTION_PATTERN = Pattern.compile(".+\\(OUI/NON\\)");
    ...
    private String answer(String question) {
        ...
        if (matcherAddition.matches()) {
            return String.valueOf(Integer.parseInt(matcherAddition.group(1))
                    + Integer.parseInt(matcherAddition.group(2)));
        }

        Matcher matcherMultiplication = MULTIPLICATION_PATTERN.matcher(question);
        if (matcherMultiplication.matches()) {
            return String.valueOf(Integer.parseInt(matcherMultiplication.group(1))
                    * Integer.parseInt(matcherMultiplication.group(2)));
        }
